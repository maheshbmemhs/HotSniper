...
void SchedulerOpen::initDVFSPolicy(String policyName) {
	...
	if (policyName == "off") {
		...
		float temperature_constraint = Sim()->getCfg()->getFloat("scheduler/open/dvfs/DynThreadMapping_dvfs/temperature_constraint");
		...
		dvfsPolicy = new DynThreadMapping_dvfs(performanceCounters,coreRows,coreColumns,std::string(profile_path.c_str()),std::string(thermal_model_path.c_str()),temperature_constraint,core_states,criticalTemp,recovery);
	} else {
